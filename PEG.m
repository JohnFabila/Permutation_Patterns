function [Out_PEG, npdf, vertexPatterns]=PEG(x,Adj,m,L)
...
if or(size(Adj,1)~=N, size(Adj,2)~=N )
    ...
    pdf=zeros(1,pat);
% Initialize a cell array to store the permutation pattern for each vertex
    vertexPatterns = cell(1, N);

    for j=1:N
        [~,index]=sort(Lap(j,:));
        vertexPatterns{j} = index; % Save the pattern associated with the vertex
        for jj=1:pat
